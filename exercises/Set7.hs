...
travel (Velocity ms) (Time s)= Distance (ms * s)
...
member m (Set set) = m `elem` set 

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add newMember (Set set)
  | member newMember (Set set)   = Set set
  | otherwise                    = Set (sort (newMember : set))
...
-- data State = Start | Error | Finished
  -- deriving (Eq,Show)
data State = Start | Eggs | Flour | Sugar | Prep2 | Stir | Finished | Error
  deriving (Eq,Show)

step :: State -> Event -> State
step Finished _ = Finished
step state AddEggs | state == Start = Eggs | otherwise = Error 
step state AddFlour
  | state == Eggs   = Flour
  | state == Sugar  = Prep2
  | otherwise       = Error
step state AddSugar
  ...
  | state == Flour  = Prep2
  | otherwise       = Error
step state Mix  | state == Prep2 = Stir     | otherwise = Error
step state Bake | state == Stir  = Finished | otherwise = Error
...
-- average (x :| xs) = foldr (+) 0 (x:xs) / fromIntegral (length (x:xs))
...
reverseNonEmpty (x :| xs) 
  | not (null xs)   = last xs :| (tail . reverse) (x:xs) 
  | otherwise       = x :| xs
...
instance Ord a => Semigroup (Set a) where
  (<>) (Set a) (Set b) = squash $ Set (a++b)

squash :: Ord a => Set a -> Set a
squash (Set x) = Set $ map (\x -> head x) ((group . sort) x)
...
passwordAllowed s (MinimumLength size) = length s >= size
passwordAllowed s (ContainsSome members) = hasMembers s members
passwordAllowed s (DoesNotContain members) = not $ hasMembers s members
passwordAllowed s (And require1 require2) = passwordAllowed s require1 && passwordAllowed s require2
passwordAllowed s (Or require1 require2) = passwordAllowed s require1 || passwordAllowed s require2

hasMembers :: String -> String -> Bool
hasMembers [] members = False
hasMembers (x:xs) members
  | x `elem` members    = True
  | otherwise           = hasMembers xs members
...
-- needs recursive data type? like tree
-- data Arithmetic = Arithmetic Integer | Operation String
data Arithmetic = Arithmetic Integer | Operation String (Arithmetic) (Arithmetic)
  deriving Show


literal :: Integer -> Arithmetic
literal c = Arithmetic c

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
-- operation s (Arithmetic a) (Arithmetic b) = Operation s (Arithmetic a) (Arithmetic b)
operation = todo

evaluate :: Arithmetic -> Integer
-- evaluate (Arithmetic a) = a
-- evaluate (Operation op (Operation op1 ar i) (Operation op2 ar2 j)) = 1
-- evaluate (Operation "*" (Arithmetic a) (Arithmetic b)) =  a * b
-- evaluate (Operation "+" (Arithmetic a) (Arithmetic b)) =  a + b
evaluate = todo
...
-- render (Operation "+" (Arithmetic a) (Arithmetic b)) = "(" ++ show a ++ "+" ++ show b ++ ")"
-- render (Operation "*" (Arithmetic a) (Arithmetic b)) = "(" ++ show a ++ "*" ++ show b ++ ")"

 -- (operation "+" (operation "+" (literal 4) (literal 8)) (operation "+" (literal 2) (literal 3)))
